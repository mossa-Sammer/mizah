...
const RemoveRejoin = ({ actions, showText, type, loading, text }) => (
  <Wrapper type={type}>
    {actions.onDelete && (
      <IconButton
        onClick={actions.onDelete.handleClick}
        type={type}
        disabled={loading}
    ...
    {actions.onEdit && (
      <IconButton
        onClick={actions.onEdit.handleClick}
        type={type}
        disabled={loading}
        aria-label="Edit member"
      >
        {!loading ? (
          <Icon icon="edit" color="cyan" width="22px" height="22px" margin="0 5px 0 0" />
        ) : (
          'Loading...'
        )}
        {showText && <p color="gray3">{loading ? 'loading' : text || type}</p>}
